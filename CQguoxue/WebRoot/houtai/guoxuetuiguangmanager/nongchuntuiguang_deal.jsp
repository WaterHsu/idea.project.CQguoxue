...
<% 
		 ...
		 Connection conn = null;
   	     String sql = "delete from guoxuetuiguang_nongchuntuiguang where number = '" + number +"'";
  	     ...
  	     String sql_title = "select * from guoxuetuiguang_nongchuntuiguang where number = '" + number +"'";
		        ...
		        if(submit.equals("删除"))
		        {
		        	 statement = conn.prepareStatement(sql);
		 	    	 statement.executeUpdate(sql);
		 	    	  ...
		 	    	  log_caozuo = "在国学推广版块农村推广栏目删除文章:" + title;
		 	    	...
		 	    	// session.setAttribute("guoxuetuiguang_nongchuntuiguang_message","delete_success");
		 			 response.sendRedirect("nongchuntuiguang.jsp");
		        }
		        else if(submit.equals("修改"))
		        {
		        	String sql2 = "select * from guoxuetuiguang_nongchuntuiguang where number = '" + number +"'";
...
<html>
  ...
  <body>
  		<div>
  			<a href="houtai/guoxuetuiguangmanager/nongchuntuiguang.jsp">返回</a>
  		</div>
  			<br><br><br><br>
  
		<div>
			<h2>修改文章</h2>
			<form  method="post" action="houtai/guoxuetuiguangmanager/nongchuntuiguang_alter.jsp">
			<% 
				while(rs.next())
				{
			%>
