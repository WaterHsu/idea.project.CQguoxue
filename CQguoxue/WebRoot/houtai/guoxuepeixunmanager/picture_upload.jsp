...
<%
  ...
  // String pic = session.getAttribute("picname").toString();
   ...
   rootPath = realPath + "\\guoxuepeixunimages\\" + time_yearmonth + "\\"; //创建文件的保存目录
   String contentType = request.getContentType(); //取得客户端上传的数据类型  
	...
	//	out.println("<a href='upload/"+saveFile+"'>打开上传文件</a>");
	   ...
	   out.println("<p>上传的数据类型不是是multipart/form-data</p>"); 
		...
		/* String numberid = session.getAttribute("numberid").toString();
		 ...
		 String ssaveFile = "guoxuepeixunimages/" + time_yearmonth + "/"  + saveFile;
	...
	//	out.println(ssaveFile.substring(0, ssaveFile.lastIndexOf('/')));
  	...
  	//     String sql = "select * from guoxuepeixun_picture order by id desc" ;
	...
	//	 String sql2 = "insert into guoxuepeixun_picture (id,number,picture_route) values(" +id+ ",'" +number+ "','" +ssaveFile+ "')";
		 
		 String sql2 = "update guoxuepeixun_picture set picture_route = '" +ssaveFile+ "' where id = " +id2+ "";
		 String sql3 = "update guoxuepeixun_picture set number = '" +number+ "' where id = " +id2+ "";
