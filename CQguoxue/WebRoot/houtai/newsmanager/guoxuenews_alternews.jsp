...
<% 
	...
	//	 leibie = new String(leibie.getBytes("iso-8859-1"),"utf-8");
		 ...
		 Connection conn = null;
   	     String sql = "delete from guoxuenews_guonei where number = '" + number +"'";
  	 ...
  	 //    ResultSet rs2 = null;
  	 	  ...
  	 	  String sql_title = "select * from guoxuenews_guonei where number = '" + number +"'";
		       ...
		       // statement = conn.prepareStatement(sql);
		        ...
		        if(submit.equals("删除"))
		        {
		        	 	...
		        	 	statement = conn.prepareStatement(sql);
		 	    	 	...
		 	    	 	log_caozuo = "在国学新闻版块删除国内新闻:" + title;
		        ...
		        {
						String sql2 = "select * from guoxuenews_guonei where number = '" + number +"'";
		        ...
		        /*		while(rs.next())
		        		{
		        			out.println("<div><table><tr><td>上传一张在左侧显示的新闻图片：</td><td><form name='upfileform' method='post' action='houtai/newsmanager/guoxuenews_uploadpic2.jsp' enctype='multipart/form-data'><input type='file' id='mpic' name='mpic'  size='50' /><br><input type='submit' name='submit' value='上传' /></form></td></tr></table></div>");
		        			out.println("<div><form name='xueyuan_news' method='post' action='houtai/newsmanager/guoxuenews_upload2.jsp'><table>");
		        			out.println("<tr><td>新闻标题:</td><td><input type='text' id='title' name='title' value='" +rs.getString("title")+ "' /></td></tr>");
		        			out.println("<tr><td>作者:</td><td><input type='text' id='author' name='author' value='" +rs.getString("author")+ "' /></td></tr>");
		        			out.println("<tr><td>图片:</td><td><input name='newsimage' value='" +rs.getString("image")+ "' /></td></tr>");
		        			out.println("<tr><td>类别:</td><td><input name='leibie_exsit' value='国内' type='text' readonly='true' /></td></tr>");
		        			out.println("<tr><td>新闻内容:</td><td><input type='input' name='number' value='" +rs.getString("number")+ "'  /></td></tr>");
		        			out.println("<tr><td></td><td><textarea name='content' style='width:800px;height:400px;visibility:hidden;'>" +rs.getString("text")+ "</textarea></td></tr>");
...
<html>
  ...
  <body>
  		...
  		<div>
  			<table>
  				<tr>
  					<td>国学新闻图片：</td>
  					<td>
  						<form name="upfileform" method="post" action="houtai/newsmanager/guoxuenews_uploadpic2.jsp" enctype="multipart/form-data">
  		...
  		<div>
  			<form name="xueyuan_news" method="post" action="houtai/newsmanager/guoxuenews_upload2.jsp">
  				<table>
  					<% 
  						while(rs.next())
  						{
  					...
  					<tr>
  						<td>图片:</td>
  						<td>
  							<input name="newsimage" value="<%=rs.getString("image") %>" />
  						</td>
  					</tr>
  					<tr>
  						<td>类别:</td>
  						<td>
  							<input name="leibie_exsit" value="国内" type="text" readonly /> 							
  						</td>
  					</tr>
  					<tr>
  						<td>新闻内容:</td>
  					...
  					<tr>
  						<td></td>
  						<td>
  							<textarea name="content" style="width:800px;height:400px;visibility:hidden;"><%=rs.getString("text") %></textarea>
  						</td>
  					</tr>
  					<tr>
  						<td></td>
