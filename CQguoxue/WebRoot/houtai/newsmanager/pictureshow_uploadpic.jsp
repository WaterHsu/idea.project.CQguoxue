...
<%
		 ...
		 String time_hourminute2 = "";
		 time_hourminute += hour1;
		 time_hourminute += ":";
		 time_hourminute += minute1;
		 time_hourminute2 += hour1;
		 time_hourminute2 += "-";
		 time_hourminute2 += minute1;
  ...
  // String pic = session.getAttribute("picname").toString();
   ...
   rootPath = realPath + "\\pictureshowimages\\" + time_yearmonth + "\\" + time_hourminute2 + "\\"; //创建文件的保存目录
   String contentType = request.getContentType(); //取得客户端上传的数据类型  
	...
	//	out.println("<a href='upload/"+saveFile+"'>打开上传文件</a>");
	   ...
	   out.println("<p>上传的数据类型不是是multipart/form-data</p>"); 
		...
		/* String numberid = session.getAttribute("numberid").toString();
		 ...
		 String ssaveFile = "pictureshowimages/" + time_yearmonth + "/" + time_hourminute2 + "/" + saveFile;
	...
	//	out.println(ssaveFile.substring(0, ssaveFile.lastIndexOf('/')));
		 ...
		 String dbUsr = "root";
  	     ...
  	     String sql = "select * from pictureshow order by id desc" ;
		 ...
		 String sql2 = "insert into pictureshow (id,number,picture_route,time_yearmonth,time_hourminute,author,add_user,xiangce) values(" +id+ ",'" +number+ "','" +ssaveFile+ "','" +time_yearmonth+ "','" +time_hourminute+ "','xx','xxx','" +xiangce_name+ "')";
