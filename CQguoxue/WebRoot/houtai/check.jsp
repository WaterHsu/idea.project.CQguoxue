...
<%
		 ...
		 int log_id = 0;
  	     ...
  	     try
  	     {
		        ...
		        while(rs.next())
		        {
		        	if(rs.getString("username").equals(username))
		        	{
		        		if(rs.getString("password").equals(password))
		        		{
		        			...
		        			mark = 1;
		        			if(rs.getInt("rights") == 1)
		        			{
		        				String log_sql2 = "insert into system_users_log(id,time_yearmonthday,time_hourminute,username,rights,caozuo,result) values(" +log_id+ ",'" +log_time_yearmonthday+ "','" +log_time_hourminute+ "','" +username+ "','超级权限/操作所有','登陆到重庆国学院网站后台','成功')";
		        			...
		        			else if(rs.getInt("rights") == 2)
		        			{
		        				String log_sql2 = "insert into system_users_log(id,time_yearmonthday,time_hourminute,username,rights,caozuo,result) values(" +log_id+ ",'" +log_time_yearmonthday+ "','" +log_time_hourminute+ "','" +username+ "','普通权限/发布 修改 删除','登陆到重庆国学院网站后台','成功')";
								log_statement2 = conn.prepareStatement(log_sql2);		
								log_statement2.executeUpdate(log_sql2);
		        				response.sendRedirect("webmanage.jsp");
		        				break;
		        			}
		        			else if(rs.getInt("rights") == 3)
		        			{
		        				String log_sql2 = "insert into system_users_log(id,time_yearmonthday,time_hourminute,username,rights,caozuo,result) values(" +log_id+ ",'" +log_time_yearmonthday+ "','" +log_time_hourminute+ "','" +username+ "','普通权限/发布','登陆到重庆国学院网站后台','成功')";
								log_statement2 = conn.prepareStatement(log_sql2);		
								log_statement2.executeUpdate(log_sql2);
		        				response.sendRedirect("webmanage.jsp");
		        				break;
		        ...
		        if(mark == 0)
		        {
					session.setAttribute("wrong","suwrong");
					response.sendRedirect("login.jsp");
				}
  	     }catch(Exception e)
  	     {
